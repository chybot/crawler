...
import QueueBase
from kafka import KafkaClient, SimpleConsumer, SimpleProducer, common,SimpleClient,KafkaProducer
...
class QueueKafka(QueueBase.QueueBase):
    def __init__(self, name, host='web14', port=51092, **kwargs):
        ...
        self.__consumer = SimpleConsumer(self.__kafka, self.name + '_consumer',
                                         self.name, auto_commit_every_n=1, max_buffer_size=None)
    ...
    def put(self, value, *args, **kwargs):
        if isinstance(value, dict) or isinstance(value, list):
            self.__producer.send_messages(self.name, json.dumps(value, ensure_ascii=False).encode('utf-8'))
        else:
